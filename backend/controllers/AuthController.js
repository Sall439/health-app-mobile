...
const loginUsers = async (req, res) => {
   try {
        ...
        if(!email || !password){
            return res.status(400).json({messages: "Veuillez remplir ses champs"})
        }
        ...
        if(!comparePassword){
            return res.status(400).json({messages: "Mot de passe incorrect"})
        }
        ...
        return res.status(200).json({messages: "Utilisateur connecte"})
